...
function getZoneInfo(tenant, iasConfig, uaadomain, logger, cb) {
  ...
  return exports.callZoneInfoAPI(uaadomain, iasConfig['btp-tenant-api'], tenant, iasConfig.clientid, logger, cb);
}
...
function callZoneInfoAPI(uaadomain, btpTenantAPI, subdomain, iasClientId, logger, cb) {
  ...
  const zoneInfoUrl = btpTenantAPI ? `${btpTenantAPI}/sap/rest/tenantLoginInfo/${subdomain}?iasClient=${iasClientId}` : `https://api.${uaadomain}/sap/rest/tenantLoginInfo/${subdomain}?iasClient=${iasClientId}`;
